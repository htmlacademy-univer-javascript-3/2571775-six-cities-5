...
function OfferCard({offer, classNameType}: OfferCardProps): JSX.Element {
  ...
  const handleOnMouseOver = useCallback(() => dispatch(setCurrentOfferId(offer.id)), [dispatch, offer.id]);
  const handleOnMouseOut = useCallback(() => dispatch(setCurrentOfferId('')), [dispatch]);
  ...
  return (
    <article className={`${classNameType}__card place-card`} onMouseOver={handleOnMouseOver} onMouseOut={handleOnMouseOut}>
      {
        offer.isPremium ?
      ...
      <div className="place-card__info">
        <div className="place-card__price-wrapper">
          ...
          <button className={`place-card__bookmark-button${offer.isFavorite ? ' place-card__bookmark-button--active' : ''} button`} type="button" onClick={handleOnClick}>
