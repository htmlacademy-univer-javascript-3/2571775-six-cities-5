...
function SortingVariables(): JSX.Element {
  ...
  return(
    <form className="places__sorting" action="#" method="get">
      <span className="places__sorting-caption" onClick={handleOnClick}>Sort by</span>
      <span className="places__sorting-type" tabIndex={0} onClick={handleOnClick}>
        {sortVersion}
      ...
      <ul className={`places__options places__options--custom${isOpened ? ' places__options--opened' : ''}`}>
        {Object.values(SortTypes).map((type) => <MemoizedSortVersion type={type} key={type} onClickCallback={handleOnClick}/>)}
      </ul>
    </form>);
}
