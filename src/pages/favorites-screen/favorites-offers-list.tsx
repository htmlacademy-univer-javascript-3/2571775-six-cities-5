...
function FavoritesOfferList(): JSX.Element {
  const offers = [...useAppSelector((state) => state.favoriteOffers)].reduce<Record<string, MainPageOffers>>((acc, item) => {
              ...
              <div className='favorites__places'>
                {
                  offer[1].map((realOffer, anotherId) => {
                    ...
                    return(<MemoizedFavoriteOffer offer={realOffer} key={keyValueAnother}/>);
                  })
                }
              </div>
            </div>);
